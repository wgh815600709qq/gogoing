...
var { add, queryAllCarousel, queryByOne, deleteByOne, editById } = require('../module/picture.js')
...
router.post('/upload', async (req, res, next) => {
  upload(req, res, async (err) => {
    ...
    // 调存储业务
    let data = {
      _path: '/uploads/' + req.file.filename
    }
...
// 查询
router.post('/queryAllCarousel', async (req, res, next) => {
  let result = await queryAllCarousel(req.body)
  if (result) {
    res.send(Object.assign(success, {data: result}))
  } else {
    res.send(Object.assign(fail, {data: null}))
...
// 删除
router.post('/dropPicture', async (req, res, next) => {
...
// 编辑
router.post('/editPicture/:id', async (req, res, next) => {
